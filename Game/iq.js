...
function showSolution() {
    ...
    for (let i = 0; i < gSizeY * gSizeX; ++i) {
        ...
        if (gBoard[i].marked !== gBoard[i].selected) {
            ...
            // Display an error message and ask the user if they want to exit or continue
            let userConfirmed = window.confirm("You made an incorrect selection. Do you want to exit the game?");
            if (userConfirmed) {
                // If the user wants to exit, reset the game
                resetGame();
                return;
            } else {
                // If they want to continue, just stop the process and allow them to fix their mistakes
                break;
            }
        } else {
...
function resetGame() {
    ...
    gLevel = 1; // Reset the level back to 1
    startRound();
}
